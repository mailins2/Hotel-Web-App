...
## Logic card Đặt phòng hôm nay

Card **Đặt phòng hôm nay** hiển thị số lượng đơn đặt phòng mới được tạo trong đúng ngày hiện tại.

Nguồn dữ liệu:

- Bảng `DatPhong`
- Trường `DatPhong.NgayDat`: ngày tạo đơn đặt phòng
...
- `$reportDefaultTo` là ngày hiện tại theo server, được tạo bằng `Carbon::today()`.
- `whereDate('NgayDat', ...)` chỉ lấy các booking có ngày đặt trùng với ngày hiện tại.
- `count()` đếm số dòng `DatPhong`, tức số đơn đặt phòng mới trong ngày đó.

Ví dụ ngày `29/05/2026` có 10 đơn mới thì card hiển thị `10`. Sang ngày `30/05/2026`, nếu chỉ có thêm 2 đơn mới thì card hiển thị `2`. Nếu trong ngày không có đơn mới, câu truy vấn không tìm thấy dòng nào có `NgayDat` bằng ngày hiện tại nên card hiển thị `0`.

Biến này được truyền vào view báo cáo:
...
Fallback `0` giúp giao diện không lỗi nếu view chưa nhận được biến.

## Logic card Số lượng phòng và Phòng trống

Hai card này hiển thị số lượng phòng hiện tại trong bảng `Phong`.

Nguồn dữ liệu:

- Bảng `Phong`
- Khóa chính `Phong.MaPhong`
- Trường `Phong.TinhTrang`

Card **Số lượng phòng** không lọc theo trạng thái. Card này đếm toàn bộ bản ghi hiện có trong bảng `Phong`, nên khi thêm một phòng mới vào hệ thống, số lượng sẽ tăng trong lần tải lại trang báo cáo tiếp theo.
...
- `$roomCount`: tổng số phòng hiện có trong bảng `Phong`, không phân biệt trạng thái.
- `$roomUsingCount`: tổng số phòng có trạng thái **Đang sử dụng**. Biến này vẫn được chuẩn bị để dùng cho các phần thống kê khác nếu cần.
- `$roomEmptyCount`: tổng số phòng có trạng thái **Trống**.

Các biến này được truyền vào cùng view báo cáo:
...
Nghĩa là khi thêm phòng mới hoặc trạng thái phòng trong database thay đổi, người dùng chỉ cần tải lại trang báo cáo. Route sẽ chạy lại truy vấn, đếm lại dữ liệu mới nhất trong bảng `Phong`, rồi render lại số mới lên card.
...
```js
occupancyRate =
    capacityRoomNights > 0 ? (soldRoomNights / capacityRoomNights) * 100 : 0;
```
...
```html
<rect data-main-chart-bar="true"></rect>
```
...
Khi xuất file, dữ liệu phòng được sắp xếp theo **Loại phòng** rồi đến **Số phòng**. Sau khi thống kê xong từng loại phòng, hệ thống thêm một dòng **Tổng cộng** cho loại phòng đó để cộng các cột **Số lượt đặt**, **Số ngày được thuê** và **Doanh thu phòng**.
...
| Giá phòng           | Giá phòng hiện tại sau khuyến mãi nếu khuyến mãi giao với kỳ báo cáo |
...
```php
roomTypePriceForPeriod(Phong.loaiPhong, $fromDate, $endExclusive)
```

Hệ thống lấy giá gốc từ:

```php
Phong.loaiPhong.GiaPhong
```

Nếu loại phòng có `MaKM` và khuyến mãi đó có thời gian hiệu lực giao với khoảng báo cáo, giá phòng trong file Excel sẽ là giá sau giảm:

```php
LoaiPhong::giaSauKhuyenMai($promotionStart)
```

Ví dụ báo cáo từ `01/05/2026` đến `31/05/2026`, loại phòng có khuyến mãi từ `10/05/2026` đến `20/05/2026` thì cột **Giá phòng** hiển thị giá đã giảm. Nếu khuyến mãi nằm ngoài kỳ báo cáo, cột này hiển thị giá gốc.
...
`Dòng Tổng cộng theo loại phòng`:

Sau mỗi nhóm loại phòng, dòng **Tổng cộng** cộng:

```php
$items->sum('booking_count')
$items->sum('rented_days')
$items->sum('room_revenue')
```

Trong đó:

- `booking_count`: tổng số lượt đặt của tất cả phòng thuộc loại phòng đó.
- `rented_days`: tổng số ngày được thuê của tất cả phòng thuộc loại phòng đó.
- `room_revenue`: tổng doanh thu phòng của tất cả phòng thuộc loại phòng đó.
